...
module RefinementProof_i {
    ...
    predicate ExistsLockPacket(glb:seq<GLS_State>, config:Config, i:int, j:int)
        requires IsValidBehavior(glb, config);
        requires 0 <= i < |glb|-1;
    {
        exists p :: p in glb[i].ls.environment.sentPackets && p.msg.Locked? && p.msg.locked_epoch == j
    }*/
    ...
    lemma MakeLockHistory(glb:seq<GLS_State>, config:Config, i:int) returns (history:seq<EndPoint>)
        ...
        //ensures |history| > 1 ==> (forall h :: h in lb[i].servers && lb[i].servers[h].held ==> ExistsLockPacket(lb, config, i, |history|));
        //ensures forall j :: 2 <= j < |history| ==> ExistsLockPacket(lb, config, i, j);
        //ensures history == makeHistory(lb[i]);
        //ensures forall epoch,h :: h in makeHistory1(lb[i])[epoch] <==> 2 <= epoch <= |history| && h == history[epoch-1];
    {
        if i == 0 {
            history := [config[0]];
        } else {
            ...
            assert forall p :: p in glb[i-1].ls.environment.sentPackets && p.msg.Locked? ==> p in glb[i].ls.environment.sentPackets;
            /*forall j | 2 <= j < |prevHistory| 
                ensures ExistsLockPacket(lb, config, i, j);
            {
                assert ExistsLockPacket(lb, config, i-1, j);
                var p :| p in lb[i-1].environment.sentPackets && p.msg.Locked? && p.msg.locked_epoch == j;
            }*/
            if s.ls.environment.nextStep.LEnvStepHostIos? && s.ls.environment.nextStep.actor in s.ls.servers {
                ...
                if NodeGrant(s.ls.servers[id], s'.ls.servers[id], ios) {
                    ...
                    history := prevHistory;
                    //assert makeHistory1(lb[i-1]) == makeHistory1(lb[i]);
