...
class NodeImpl
{
    ...
    function AbstractifyNodeImpl() : Node 
        reads this;
    {
        AbstractifyCNode(node)
    }
            ...
            && old(Env().udp.history()) + udpEventLog == Env().udp.history());
    {
        ...
        node,sent_packets := HostModelSpontaneouslyRetransmit(node);
    ...
    method HostNextReceivePacket(ghost udpEventLogOld:seq<UdpEvent>, rr:ReceiveResult, ghost receive_event:UdpEvent) returns (ok:bool, ghost udpEventLog:seq<UdpEvent>, ghost ios:seq<LSHTIo>)
        ...
        requires rr.cpacket.dst == node.me;
        modifies Repr;
        ensures Repr == old(Repr);
        ensures ok == UdpClientOk(udpClient);
        ensures Env() == old(Env());
            ...
            && udpEventLogOld + udpEventLog == Env().udp.history());
    {
        ...
        node, sent_packets, ack := HostModelReceivePacket(node, cpacket); 
        assert Valid();
        ...
        assert Env() == old(Env());
        assert Valid();
        assert udpClient.LocalEndPoint() == node.me;
...
//                assert LHost_ReceivePacketWithoutReadingClock(old(AbstractifyToHost()), AbstractifyToHost(), ios);
    ...
    method{:timeLimitMultiplier 2} Host_ProcessReceivedPacket_Next() returns (ok:bool, ghost udpEventLog:seq<UdpEvent>, ghost ios:seq<LSHTIo>)
        ...
        ensures ok ==> (
            ...
            && (LHost_ProcessReceivedPacket_Next(old(AbstractifyToHost()), AbstractifyToHost(), ios)
                || HostNextIgnoreUnsendableProcess(old(AbstractifyToLNode()), AbstractifyToLNode()[nextActionIndex := 2], udpEventLog))
            && old(AbstractifyToHost()).me == AbstractifyToHost().me
            && RawIoConsistentWithSpecIO(udpEventLog, ios)
            && OnlySentMarshallableData(udpEventLog) 
            && LIoOpSeqCompatibleWithReduction(ios)
            && old(AbstractifyToLNode()).node.constants == AbstractifyToLNode().node.constants
            && old(Env().udp.history()) + udpEventLog == Env().udp.history());
    {
        var sent_packets := [];
        var b;
        if (node.receivedPacket.Some?)
        {
        
            b := ShouldProcessReceivedMessageImpl(node);
            if (b) {
                var cpacket := node.receivedPacket.v;

                if (cpacket.msg.CSingleMessage?) {
                    assert |node.delegationMap.lows| < 0xFFFF_FFFF_FFFF_FFFF - 2;
                    node, sent_packets := HostModelNextReceiveMessage(node, cpacket);
                } else {
                    node := node[receivedPacket := None];
                    sent_packets := [];
                    assert false;
                }
            } else {
        ...
        if (old(AbstractifyToHost()).receivedPacket.Some?) {
            if (b) {
                ...
                if HostIgnoringUnParseable(old(AbstractifyToHost()), AbstractifyToHost(), AbstractifySeqOfCPacketsToSetOfShtPackets(sent_packets)) {
                    assert HostNextIgnoreUnsendableProcess(old(AbstractifyToLNode()), AbstractifyToLNode()[nextActionIndex := 2], udpEventLog);
                }
                assert LHost_ProcessReceivedPacket_Next(old(AbstractifyToHost()), AbstractifyToHost(), ios)
                    || HostNextIgnoreUnsendableProcess(old(AbstractifyToLNode()), AbstractifyToLNode()[nextActionIndex := 2], udpEventLog);
            } else {
    ...
    method {:timeLimitMultiplier 2} Host_Next_main()
        ...
        ensures ok ==> (
               Valid()
            && (   LNode_Next(old(AbstractifyToLNode()), AbstractifyToLNode(), ios)
                || HostNextIgnoreUnsendable(old(AbstractifyToLNode()), AbstractifyToLNode(), udpEventLog))
            && LIoOpSeqCompatibleWithReduction(ios)
            && RawIoConsistentWithSpecIO(udpEventLog, ios)
            && OnlySentMarshallableData(udpEventLog) 
            && old(Env().udp.history()) + udpEventLog == Env().udp.history()
            );
    {
        ...
        var curResendCount;
        var nextResendCount;
        ghost var node_old := old(AbstractifyToHost());
        ghost var scheduler_old := old(AbstractifyToLNode());
        ghost var node;
        ghost var scheduler;

        //print ("Host_Next_main Enter\n");
        assert scheduler_old.node == node_old;
        if (curActionIndex == 0) {
        ...
        node := AbstractifyToHost();
        nextActionIndex := nextActionIndex';
        scheduler := AbstractifyToLNode();
        calc {
        ...
        if (curActionIndex == 2) {
            ...
            if (nextResendCount == 0) {
                assert LHost_NoReceive_Next(old(AbstractifyToLNode()).node, AbstractifyToLNode().node, ios);
            } else {
        ...
        //assert  LHost_ReceivePacket_Next(old(AbstractifyToLNode()).host, AbstractifyToLNode().host, ios) || LHost_ProcessReceivedPacket_Next(old(AbstractifyToLNode()).host, AbstractifyToLNode().host, ios) || LHost_NoReceive_Next(old(AbstractifyToLNode()).host, AbstractifyToLNode().host, ios);
        assert UdpClientIsValid(udpClient);
        assert old(AbstractifyToLNode()).node.constants == AbstractifyToLNode().node.constants; 
        assert {:split_here} true;
        if (curActionIndex == 0) {
            assert old(AbstractifyToLNode()).nextActionIndex == 0;
            calc {
                AbstractifyToLNode().nextActionIndex;
                int(curActionIndex+1) % LHost_NumActions();
            ...
            assert LNode_Next(old(AbstractifyToLNode()), AbstractifyToLNode(), ios)
                || HostNextIgnoreUnsendable(old(AbstractifyToLNode()), AbstractifyToLNode(), udpEventLog);
        } else if (curActionIndex == 1) {
            assert LNode_Next(old(AbstractifyToLNode()), AbstractifyToLNode(), ios)
                || HostNextIgnoreUnsendable(old(AbstractifyToLNode()), AbstractifyToLNode(), udpEventLog);
