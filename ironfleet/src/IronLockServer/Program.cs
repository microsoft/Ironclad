...
");
    ...
    static void Main(string[] args)
    {
      ...
      Dafny.ISequence<byte>[] serverPublicKeys =
        serviceIdentity.Servers.Select(server => Dafny.Sequence<byte>.FromArray(BitConverter.GetBytes(ByteArrayComparer.Default().GetHashCode(server.PublicKey)))).ToArray();
